...
myTestData.columns = ['age', 'sex', 'chest_pain_type', 'resting_blood_pressure', 'cholesterol', 'fasting_blood_sugar', 'rest_ecg', 'max_heart_rate_achieved', 'exercise_induced_angina', 'st_depression', 'st_slope', 'num_major_vessels', 'thalassemia', 'target']
...
myTestData['sex'][myTestData['sex'] == 0] = 'female'
myTestData['sex'][myTestData['sex'] == 1] = 'male'
myTestData['chest_pain_type'][myTestData['chest_pain_type'] == 1] = 'typical angina'
myTestData['chest_pain_type'][myTestData['chest_pain_type'] == 2] = 'atypical angina'
myTestData['chest_pain_type'][myTestData['chest_pain_type'] == 3] = 'non-anginal pain'
myTestData['chest_pain_type'][myTestData['chest_pain_type'] == 4] = 'asymptomatic'
myTestData['fasting_blood_sugar'][myTestData['fasting_blood_sugar'] == 0] = 'lower than 120mg/ml'
myTestData['fasting_blood_sugar'][myTestData['fasting_blood_sugar'] == 1] = 'greater than 120mg/ml'
myTestData['rest_ecg'][myTestData['rest_ecg'] == 0] = 'normal'
myTestData['rest_ecg'][myTestData['rest_ecg'] == 1] = 'ST-T wave abnormality'
myTestData['rest_ecg'][myTestData['rest_ecg'] == 2] = 'left ventricular hypertrophy'
myTestData['exercise_induced_angina'][myTestData['exercise_induced_angina'] == 0] = 'no'
myTestData['exercise_induced_angina'][myTestData['exercise_induced_angina'] == 1] = 'yes'
myTestData['st_slope'][myTestData['st_slope'] == 1] = 'upsloping'
myTestData['st_slope'][myTestData['st_slope'] == 2] = 'flat'
myTestData['st_slope'][myTestData['st_slope'] == 3] = 'downsloping'
myTestData['thalassemia'][myTestData['thalassemia'] == 1] = 'normal'
myTestData['thalassemia'][myTestData['thalassemia'] == 2] = 'fixed defect'
myTestData['thalassemia'][myTestData['thalassemia'] == 3] = 'reversible defect'

myData = pd.get_dummies(myData, drop_first=True)
myTestData = pd.get_dummies(myTestData, drop_first=True)
...
myTestData = (myTestData - np.min(myTestData)) / (np.max(myTestData) - np.min(myTestData))
x_train, x_test, y_train, y_test = train_test_split(myData.drop('target', axis=1),
                                                    myData['target'], test_size=.2, random_state=0)
p_train = x_train
q_train = y_train
x_train, x_test, y_train, y_test = train_test_split(myTestData.drop('target', axis=1),
                                                    myTestData['target'], test_size=0.1, random_state=0)
x_train = p_train
y_train = q_train
